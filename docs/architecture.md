...
- **项目 Git 仓库地址必填并校验**：创建项目必须提供 `git_repo_url`，编辑项目时不允许清空；前端即时校验，后端是最终防线。支持 `https://host/org/repo(.git)`、`ssh://user@host/org/repo.git`、`git@host:org/repo.git`；GitHub/GitLab/Bitbucket/Codeberg/Gitee 等已知 Web Git host 允许不带 `.git` 的仓库根地址，未知 host 要求仓库路径以 `.git` 结尾。拒绝 issues/pull/tree/blob 等页面 URL、query/fragment、内嵌凭据、私有/本地/metadata IP、危险协议和 leading dash。这是格式与安全校验，不是仓库存在性或可访问性校验
- **时间语义分两组**：
